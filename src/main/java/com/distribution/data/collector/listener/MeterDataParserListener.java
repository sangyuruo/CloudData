...
public class MeterDataParserListener implements GenericApplicationListener, ApplicationEventPublisherAware {
    ...
    public void onApplicationEvent(ApplicationEvent event) {
        ...
        if (list.size() > 0) {
            ...
            List<SmartMeterDataMsg> msgs = new ArrayList<>();
            for (SmartMeterData data : list) {
                SmartMeterDataMsg newMsg = new SmartMeterDataMsg();
                BeanUtils.copyProperties(data, newMsg);
                newMsg.setId(data.getId().toString());
                newMsg.setServerId(data.getServerId().toString());
                newMsg.setMeterId(data.getMeterId().toString());
                newMsg.setCompanyId(data.getCompanyId().toString());
                msgs.add(newMsg);
            }
            producer.send(new MeterDataMsgEvent("create", msgs));
        }
    }
