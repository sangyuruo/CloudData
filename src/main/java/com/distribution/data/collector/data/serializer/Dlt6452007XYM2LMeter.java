...
@Component
public class Dlt6452007XYM2LMeter implements IMeterParser {
	...
	private final Logger logger = LoggerFactory.getLogger(Dlt6452007XYM2LMeter.class);
        ...
        if("2007".equalsIgnoreCase(s)){
            for (String key : data.keySet()) {
                if("0000FF00".equalsIgnoreCase(key)){ //电量  4字节，x 800
                    ...
                    int st = Integer.parseInt(Dlt645Utils.toReverseHexString(data.get(key), 0, 1));
                    dataMap.put("St", Float.valueOf(st));
                    int recordtime = Integer.parseInt(Dlt645Utils.toReverseHexString(data.get(key), 1, 1));
                    dataMap.put("Trip", Float.valueOf(recordtime));
