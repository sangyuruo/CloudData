...
@Component
public class YGEnergyMeter implements IMeterParser {
	public static final String[] strKeys = new String[]{"U_a", "U_b", "U_c", "U_ab", "U_bc", "U_ca", "I_a", "I_b", "I_c", "P_a", "P_b", "P_c", "P_s", "Q_a", "Q_b", "Q_c", "Q_s", "S_s", "P_f_s", "F_f", "W_p"};
	private final Logger logger = LoggerFactory.getLogger(YGEnergyMeter.class);
	@Override
	public void parser(Map<String, Float> batchArgs, byte[] data, int start, int numberOfReg, Map<String, String> calculates, Map<String, String> auxiliary) {
		if(data != null && data.length == numberOfReg * 2 && validate(start, numberOfReg)){
	    	...
	    	for (; i < length; i++) {
				...
				try {
					int k = i * 4;
					if(key.equals("U_a") || key.equals("I_a") || key.equals("P_a") || key.equals("Q_a") || key.equals("F_f") || key.equals("E_p")) {
